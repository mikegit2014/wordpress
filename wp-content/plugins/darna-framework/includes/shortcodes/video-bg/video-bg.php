...
if (!class_exists('g5plusFramework_Shortcode_Video_Bg')) {
    class g5plusFramework_Shortcode_Video_Bg
    {
        ...
        function video_bg_shortcode($atts)
        {
            ...
            ob_start();?>
            <div class="darna-video-bg <?php echo esc_attr($layout_style); ?> <?php echo esc_attr($g5plus_animation) ?>" <?php echo g5plusFramework_Shortcodes::g5plus_get_style_animation($duration,$delay); ?>>
                <?php if($autoplay !='yes'): ?>
                <div class="video-play video-bg-img" style="background-image: url(<?php echo esc_url($img[0]); ?>);"></div>
                <?php endif;?>
                <div class="video-play video-bg-video" <?php echo wp_kses_post($style_autoplay); ?>>
                    <video id="video<?php echo esc_attr($number_id) ?>" muted="muted" loop="loop" preload="auto" <?php if($autoplay == 'yes') echo 'autoplay'; ?> poster="<?php echo esc_url($img[0]); ?>">
