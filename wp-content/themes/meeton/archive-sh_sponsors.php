<?php global $wp_query; //meeton_printr($wp_query); 
$options = _WSH()->option();
get_header();
...
$sidebar = meeton_set( $settings, 'archive_page_sidebar', 'blog-sidebar' );
_WSH()->page_settings = array('layout'=>$layout, 'view'=> $view, 'sidebar'=>$sidebar);
$classes = ( !$layout || $layout == 'full' || meeton_set($_GET, 'layout_style')=='full' ) ? ' col-lg-12 col-md-12 col-sm-12 col-xs-12 ' : ' col-lg-8 col-md-8 col-sm-7 col-xs-12 ' ;
if($layout == 'both') $classes = ' col-lg-6 col-md-6 col-sm-6 col-xs-12 ';  
$title = meeton_set($options, 'archive_title');
$bg = meeton_set($options, 'archive_bg');
?>
