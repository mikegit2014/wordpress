...
$layout = meeton_set( $settings, 'search_page_layout', 'right' );
$sidebar = meeton_set( $settings, 'search_page_sidebar', 'blog-sidebar' );
$view = meeton_set( $settings, 'search_page_view', 'list' );
...
$title = meeton_set($options, 'search_title');
$bg = meeton_set($options, 'search_bg');
?>
<!-- Page Banner -->
	...
	<!-- sidebar area -->
		...
		<?php else : ?>
					<div class="<?php echo esc_attr($classes);?> search_post_area">
						<p><?php esc_html_e( 'It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', 'meeton' ); ?></p>
						<aside>
						<?php get_search_form(); ?>
						</aside>
					</div>
